function res = vl_simplenn(net, x, dzdy, res, type, varargin)
...
% Modifications: A new input variable 'type' that should be 'train' or
% 'reg'. When 'type' is 'train', nothing is changed compared to the
% original code. When 'type' is 'reg', DZDY is replaced by res(n+1).x in
% the beginning of the backward pass. This code is to be use to implement a
% new scheme of regularization based on function norm. 
%-------------------------------------------------------------------------
...
if doder
%---------------------------------------    
% Modified code by ART    
  switch type
      case 'train'
        res(n+1).dzdx = dzdy;
      case 'reg' 
         res(n+1).dzdx = res(n+1).x;
      otherwise 
          error('Unknown type. Please review your inputs');
  end;
%---------------------------------------  
